...
toC :: Hom (U (F Unit)) (U (F U64)) -> Doc Style
toC x = case fold x of
  C y ->
    pretty rt
      <> hardline
  ...
  LetVal f x -> do
    ...
    pure $
      dent $
        vsep
          [ pretty "{",
            sep [pretty "void *", v, pretty "=", x', pretty ";"],
            body,
            pretty "}"
          ]
  ThunkVal x -> do
    body <- act x
    pure $ dent $ vsep [pretty "(struct thunk){", body, pretty "}"]
  FanoutVal x y -> do
    x' <- val x
    y' <- val y
    pure $ sep [pretty "(struct pair){", x', pretty ",", y', pretty "}"]
  IntrinsicVal intrins (FanoutVal x y) -> do
    x' <- val x
    y' <- val y
    case intrins of
      AddIntrinsic -> pure $ parens $ sep [x', pretty "+", y']
      MulIntrinsic -> pure $ parens $ sep [x', pretty "*", y']
  IntrinsicVal intrins arg -> do
    arg' <- val arg
    case intrins of
      AddIntrinsic -> pure (pretty "add" <> parens arg')
      MulIntrinsic -> pure (pretty "mul" <> parens arg')

act :: Act V V a -> State Int (Doc Style)
act x = case x of
  VarAct (V x) -> pure x
  EmptyAct -> pure $ pretty "0"
  DropAct x -> do
    x' <- act x
    pure (x' <> pretty ".snd")
  PushAct h t -> do
    h' <- val h
    t' <- act t
    pure $ sep [pretty "(struct push){", h', pretty ",", t', pretty "}"]
  PopAct f x -> do
    v <- fresh
    x' <- act x
    let h = v <> pretty ".fst"
  ...
  CallAct _ pkg name -> do
    pure (pretty pkg <> pretty "_" <> pretty name)
