...
asRight :: Term stack code => code a b -> Closed a b
asRight x = Closed (outc (foldCode x))
...
data PC k a b where
  ...
  (:.:) :: (KnownSet a, KnownSet b, KnownSet c) => PC k b c -> k a b -> PC k a c

data PK stack a b where
  Skip :: KnownAlgebra a => PK stack a a
  (:<<<:) :: (KnownAlgebra a, KnownAlgebra b, KnownAlgebra c) => PK k b c -> k a b -> PK k a c
